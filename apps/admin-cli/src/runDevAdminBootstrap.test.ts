...
import {
  runDevAdminBootstrap,
  type DevAdminBootstrapDependencies,
} from "./runDevAdminBootstrap.js";

const secretArn =
  "arn:aws:secretsmanager:us-west-2:111111111111:secret:cpi/dev/admin-bootstrap/123-1-AbCd12";

describe("runDevAdminBootstrap", () => {
  it("creates one DEV administrator without rerunning migrations", async () => {
    const output: string[] = [];
    ...
    const exitCode = await runDevAdminBootstrap(
    ...
    expect(dependencies.createAdmin).toHaveBeenCalledWith({
      environment: expect.objectContaining({ CPI_DEPLOYMENT_STAGE: "dev" }),
      input: {
        email: "Admin@Example.com",
        password: "a unique password phrase",
      },
    ...
    expect(output).toEqual(["DEV administrator bootstrap completed.\n"]);
  });

  it.each([
    ["production stage", { CPI_DEPLOYMENT_STAGE: "production" }],
    ...
    expect(output).toEqual([
      "DEV administrator bootstrap failed: BOUNDARY_REJECTED.\n",
    ]);
  });
  ...
  it.each([
    ...
    expect(output).toEqual([
      "DEV administrator bootstrap failed: SECRET_REJECTED.\n",
    ]);
  });
  ...
  it("does not expose credentials when persistence fails", async () => {
    ...
    expect(exitCode).toBe(1);
    expect(output).toEqual([
      "DEV administrator bootstrap failed: PERSISTENCE_FAILED.\n",
    ]);
    expect(output.join(" ")).not.toContain("secret-value");
  });

  it("reports an unavailable secret without exposing the provider error", async () => {
    const output: string[] = [];
    const dependencies = createDependencies(validSecret());
    vi.mocked(dependencies.readSecret).mockRejectedValueOnce(
    ...
    expect(dependencies.createAdmin).not.toHaveBeenCalled();
    expect(output).toEqual([
      "DEV administrator bootstrap failed: SECRET_UNAVAILABLE.\n",
    ]);
    expect(output.join(" ")).not.toContain("secret-value");
  });
  ...
  it.each([
    ...
    expect(exitCode).toBe(1);
    expect(output).toEqual([
      `DEV administrator bootstrap failed: ${code}.\n`,
    ]);
  });
});
...
function createRuntime(
  ...
  return {
    environment: {
      ...
      CPI_ADMIN_BOOTSTRAP_CONFIRMATION: "create-dev-admin",
...
function createDependencies(secret: string): DevAdminBootstrapDependencies {
  return {
