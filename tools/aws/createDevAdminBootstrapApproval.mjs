...
export function createDevAdminBootstrapApproval({ commit, rawPlan }) {
...
export function renderDevAdminBootstrapApproval(approval) {
  return [
    "# DEV Administrator Bootstrap Approval",
    "",
    ...
    "",
    "A create run must recompute this exact digest against unchanged DEV runtime state.",
    "",
  ].join("\n");
}
...
async function main() {
  ...
  process.stdout.write(renderDevAdminBootstrapApproval(approval));
  if (
    args.expectedDigest !== undefined &&
    args.expectedDigest !== approval.approvalDigest
  ) {
    process.stderr.write(
      "Approved DEV administrator plan does not match this commit and current runtime state.\n",
    );
    process.exitCode = 2;
  }
}
...
function validatePlan(value) {
  ...
  if (value.schemaVersion !== 1 || value.stage !== "dev") {
  ...
  if (
    ...
    throw new Error("DEV schedules must be disabled");
  }
  return value;
}
